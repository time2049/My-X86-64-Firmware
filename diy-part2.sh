...
# 🌟 核心开机自启：使用通配符匹配 /root/ 下的 luci-app-openclash*.apk
# 这样无论官方更新到什么版本号，首次开机都能精准识别并强制本地安装
...
# 5. 🌟 预埋编译期下载：在编译固件时，精准抓取官方 Release 里的最新原名 .apk 包放入 /root
echo "📥 正在编译期在线抓取 OpenClash 官方最新原生 .apk 包..."
mkdir -p package/base-files/files/root
curl -L --retry 3 https://api.github.com/repos/vernesong/OpenClash/releases/latest -o /tmp/oc_version
if [ -f "/tmp/oc_version" ]; then
    ...
    if [ -n "$download_url" ]; then
        # 提取 URL 中的原始文件名（例如 luci-app-openclash-0.47.116.apk），保持原名保存
        file_name=$(basename "$download_url")
        curl -L --retry 3 "$download_url" -o "package/base-files/files/root/$file_name"
...
# 6. 🌟 纯 nftables 架构依赖全家桶（对齐你的 [nftables for apk] 脚本）
cat << 'EOF' >> .config
